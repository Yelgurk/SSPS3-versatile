#ifndef UIMenuList_hpp
#define UIMenuList_hpp

#include "../UIElement.hpp"

class UIMenuList : public UIElement
{
public:
    UIMenuList(
        lv_obj_t * lv_screen,
    ...
    }
    {
        ...
        static uint16_t width = 160;

        lv_obj_set_width(get_container(), width);
        lv_obj_set_height(get_container(), height);
        lv_obj_align(get_container(), LV_ALIGN_LEFT_MID, 10, 30);
        ...
        remember_child_element("[menu_list]", lv_list);
        set_childs_presenter("[menu_list]");
    }
};
