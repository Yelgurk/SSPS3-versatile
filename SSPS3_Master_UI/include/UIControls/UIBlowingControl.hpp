...
public:
    ...
    }
    {
        ...
        lv_label_set_text(blow_selector_header, Translator::get(DOSING_TEMPLATES).c_str());
    ...
    void set_blow_value(float ms_aim, float ms_gone, BlowingType type, float ml_per_ms, bool reversed_indicator = true)
    {
        ...
        if (type == BlowingType::LITER)
            sprintf(buffer, ("%.3f " + Translator::get(L_LETTER)).c_str(), (ml_per_ms * (ms_aim - ms_gone)) / 1000.f);
        else
